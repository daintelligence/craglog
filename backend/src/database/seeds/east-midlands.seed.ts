...
export async function seedEastMidlands(ds: DataSource) {
  ...
  const charnwoodForest = await b('Forest Outcrops', charnwood, 2);
  ...
  const creswellMain  = await b('Gorge Wall', creswell, 1);
  const creswellCave  = await b('Cave Wall Sector', creswell, 2);

  for (const route of [
    sportRoute('Creswell Classic',  '6b',  creswellMain, { height: 14, description: 'The accessible classic of Creswell Crags — a sustained sport route on compact Magnesian limestone in the famous Ice Age gorge.' }),
    ...
    sportRoute('Gorge Route',       '7a',  creswellMain, { height: 16, description: 'A technical route on the gorge wall — sustained face climbing on compact Magnesian limestone with a serious crux section.' }),
    sportRoute('Creswell Sport',    '7b',  creswellMain, { height: 16, description: 'The testpiece of Creswell Crags — the hardest commonly climbed route in the gorge, requiring strength and precision on compact limestone.' }),
  ...
  const whatstandMain  = await b('Main Crag', whatstandwell, 1);
  const whatstandRight = await b('Right Wall', whatstandwell, 2);
  ...
  for (const route of [
    ...
    sportRoute('Short Sport',         '6c',  whatstandRight, { height: 10, description: 'A short sport route on the right wall — powerful moves on compact limestone with a distinct crux sequence.' }),
    tradRoute('Direct',               'VS',  '4c', whatstandMain,  { height: 12, description: 'A classic direct line — well protected and sustained on the main limestone face above the Derwent Valley.' }),
  ]) await r({ ...route, buttressId: route.buttress.id });
  ...
  const rainsterMain  = await b('Main Rocks', rainster, 1);
  const rainsterArch  = await b('Natural Arch Area', rainster, 2);
  ...
  for (const route of [
    ...
    tradRoute('Natural Arch Route',    'HVS', '5a', rainsterArch, { height: 12, description: 'Takes the line through the remarkable natural arch feature — a unique and atmospheric route on the White Peak plateau.' }),
    tradRoute('Lower Wall',            'S',   '4a', rainsterMain, { height: 12, description: 'An accessible moderate on the main rocks — pleasant climbing on rough natural limestone above the White Peak.' }),
  ]) await r({ ...route, buttressId: route.buttress.id });
  ...
  const aldwarkTop    = await b('Top Section', aldwark, 2);
  ...
  for (const route of [
    ...
    sportRoute('Top Route',      '7b+', aldwarkTop,  { height: 16, description: 'The hardest route at Aldwark — a demanding sequence on the steepest part of the upper section. A serious Derbyshire sport climbing challenge.' }),
  ]) await r({ ...route, buttressId: route.buttress.id });

  console.log('  ✓ East Midlands: Charnwood Forest, Creswell Crags, Whatstandwell, Rainster Rocks, Aldwark Quarry');
}
